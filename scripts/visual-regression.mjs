...
const PREVIEW_BASE = (process.env.PREVIEW_BASE || "").trim();
const DEFAULT_BASE = "http://127.0.0.1:53275";
const BASE = PREVIEW_BASE || DEFAULT_BASE;
...
for(const u of linesIn){
  ...
  try{
    ...
    console.error("Hint: run `npm run preview:check` and set PREVIEW_BASE to the printed base URL.");
    throw e;
  }
